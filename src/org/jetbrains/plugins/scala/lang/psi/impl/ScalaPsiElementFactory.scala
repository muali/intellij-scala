...
object ScalaPsiElementFactory {
  ...
  def createEquivMethodCall(infixExpr: ScInfixExpr): ScMethodCall = {
    ...
    val typeArgsText = infixExpr.getInvokedExpr match {
    ...
    val exprText = s"($baseText).$opText$typeArgsText$clauseText"
    ...
    val ref =  methodCallExpr.getInvokedExpr match {
      case gen: ScGenericCall => gen.referencedExpr.asInstanceOf[ScReferenceExpression]
      case re: ScReferenceExpression => re
    }
    ref.qualifier.get.replaceExpression(exprA, removeParenthesis = true)
    methodCallExpr
  }
